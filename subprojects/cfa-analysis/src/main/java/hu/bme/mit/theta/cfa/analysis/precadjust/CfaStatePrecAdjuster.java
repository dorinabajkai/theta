...
import hu.bme.mit.theta.analysis.prod2.PredXExpl.StatePrecAdjuster;
...
public class CfaStatePrecAdjuster implements PrecAdjuster<CfaState<Prod2State<PredState, ExplState>>, CfaAction, CfaPrec<Prod2Prec<PredPrec, ExplPrec>>> {
	private int limit;
	private final CfaLts lts;
	private Solver solver;

	private CfaStatePrecAdjuster(int limit, final CfaLts lts, Solver solver) {
		this.limit = limit;
		this.lts = lts;
		this.solver = solver;
	}

	public static CfaStatePrecAdjuster create(final Solver solver, final int limit, final CfaLts lts){
		return new CfaStatePrecAdjuster(limit, lts, solver);
	}
	...
	public CfaPrec<Prod2Prec<PredPrec, ExplPrec>> adjust(CfaPrec<Prod2Prec<PredPrec, ExplPrec>> prec, ArgNode<CfaState<Prod2State<PredState, ExplState>>, CfaAction> node) {
		...
		/*StatePrecAdjuster precAdjuster = StatePrecAdjuster.create(solver, limit, lts, (CfaState<Prod2State<PredState, ExplState>> cfaState) -> cfaState.getState().getState2());
